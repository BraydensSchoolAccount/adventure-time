...
def game(*global_vars):
    # bool typing, truthy and falsy taught
    ...
    def command_check(command):
        ...
        match command:
            ...
            case "exit":
                ...
                confirmation = get_input("Yes!", "WAIT, No!", allow_commands=False)
                if confirmation == 1:
                    print("Okay, bye!")
                    exit()
            case "help" | "?":
...
"""
    ...
    def fail(
        ...
        end(completed_fails=completed_fails, fails=fails)
    def ending(
        message,
        completed_endings=completed_endings,
        endings=endings
        ...
        os.system("cls")
        print(f"{END_BANNER}\n{message}")
        # Check if the ending has been done already
